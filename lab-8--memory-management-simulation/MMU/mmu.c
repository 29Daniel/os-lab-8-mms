...
// Helper function for First Fit policy
node_t* find_block_first_fit(list_t* freelist, int blocksize) {
...
// Helper function for Best Fit policy
node_t* find_block_best_fit(list_t* freelist, int blocksize) {
    node_t* current = freelist->head;
    node_t* best = NULL;
    int best_size = -1;
    while (current != NULL) {
        int current_size = current->blk->end - current->blk->start + 1;
        if (current_size >= blocksize && (best == NULL || current_size < best_size)) {
            best = current;
...
// Helper function for Worst Fit policy
node_t* find_block_worst_fit(list_t* freelist, int blocksize) {
    node_t* current = freelist->head;
    node_t* worst = NULL;
    int worst_size = -1;
    while (current != NULL) {
        int current_size = current->blk->end - current->blk->start + 1;
        if (current_size >= blocksize && (worst == NULL || current_size > worst_size)) {
            worst = current;
            worst_size = current_size;
        }
        current = current->next;
    }
    return worst; // Return the worst-fit block
}
...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    ...
    if (policy == 1) {
        best = find_block_first_fit(freelist, blocksize);
    } else if (policy == 2) {
        best = find_block_best_fit(freelist, blocksize);
    } else if (policy == 3) {
        best = find_block_worst_fit(freelist, blocksize);
    }
