...
void list_add_ascending_by_address(list_t *l, block_t *newblk) {
    ...
    // Handle the case where the list is empty or the new block goes at the head
    if (l->head == NULL || newblk->start < l->head->blk->start) {
        newNode->next = l->head; // New node becomes the head
        l->head = newNode;
        return;
    }
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk) {
    // Calculate the blocksize for the new block
    ...
    if (l->head == NULL || !compareSize(newblk_size, l->head->blk)) {
        newNode->next = l->head; // Insert the new node at the head
        l->head = newNode;
        return;
    }
...
void list_coalese_nodes(list_t *l) {
    if (l->head == NULL || l->head->next == NULL) {
        // If the list is empty or has only one node, no coalescing is needed
        return;
    }

    node_t *prev = l->head;         // Start from the head
    node_t *current = prev->next;  // Start with the second node

    while (current != NULL) {
        /*
         * Check if the blocks are physically adjacent
         * i.e., prev.END + 1 == current.START
         */
        if (prev->blk->end + 1 == current->blk->start) {
            // Combine the blocks by extending prev.END to current.END
            prev->blk->end = current->blk->end;

            // Skip over the current node by updating prev.NEXT
            ...
            // Advance prev and current to the next pair of nodes
            prev = current;
            current = current->next;
        }
    }
...
bool list_is_in_by_pid(list_t *l, int pid) {
    ...
    return false; // No matching PID found
}
